...
const itemContainer = document.querySelector('.shop-items') as HTMLElement;
...
function InitCart() {
    ...
    let removeCartItemButtons = document.getElementsByClassName('btn-delete') as HTMLCollection;
    ...
    let quantityField = document.getElementsByClassName('cart-quantity-input') as HTMLCollection;
    ...
    let addToCartButtons = document.getElementsByClassName('shop-item-button') as HTMLCollection;
    for (let index = 0; index < addToCartButtons.length; index++) {
        let button = addToCartButtons[index];
        button.addEventListener('click', addToCart);
    }
...
function addItemToCart(title, price, imageSrc) {
    let cartRow = document.createElement('div') as HTMLElement;
    cartRow.classList.add('cart-row');
    let cartItems = document.getElementsByClassName('cart-items')[0];
    let cartItemNames = cartItems.getElementsByClassName('cart-item-title') as HTMLCollection;
...
function updateTotal() {
    let cartItemContainer = document.getElementsByClassName('cart-items')[0] as HTMLElement;
    let cartRows = cartItemContainer.getElementsByClassName('cart-row');
    let total = 0;
    for (let i = 0; i < cartRows.length; i++) {
        let cartRow = cartRows[i];
        let priceElement = cartRow.getElementsByClassName('cart-price')[0] as HTMLElement;
